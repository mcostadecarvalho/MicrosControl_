...
#define SAMPLING_PERIOD_MS  ((uint8_t)100u) // Periodo de muestreo para sensores y ADC
 ...
 * 
 */
#define PSC              ((uint16_t)16u - 1u)  // Prescaler: divide 16 MHz a 1 MHz
...
TIM_type htim3 = { tim3, ARR, up, PSC };
...
GPIO_Init_type pir1 = {PIN_PIR1, GPIOB, PULL_DOWN, GPIO_INPUT};
GPIO_Init_type pir2 = {PIN_PIR2, GPIOB, PULL_DOWN, GPIO_INPUT};
GPIO_Init_type pir3 = {PIN_PIR3, GPIOB, PULL_DOWN, GPIO_INPUT};
GPIO_Init_type pir4 = {PIN_PIR4, GPIOB, PULL_DOWN, GPIO_INPUT};

GPIO_Init_type button = {PIN_BUTTON, GPIOB, PULL_DOWN, GPIO_INPUT}; // Pulsador NC, por lo que se configura con pull-down para leer 1 lógico cuando no está presionado. 

GPIO_Init_type relay = {PIN_RELAY, GPIOB, PUSH_PULL, GPIO_OUTPUT};
GPIO_Init_type led_alarma = {PIN_LED_ALARMA, GPIOB, PUSH_PULL, GPIO_OUTPUT};
...
static volatile uint8_t rg_counter_sampling = 0u; // Tick lógico para FSM/ADC
static volatile uint32_t rg_counter_debounce = 0u; // Contador para debounce del botón
static volatile uint32_t rg_conter_sms = 0u; // Contador para reenvío de SMS cada 2 minutos
    ...
    while (1) {
        ...
        switch (state) {
            case INIT:
                ...
                alarm_sms_sent = 0u;
                enviar_sms_uart((uint8_t *)"AT Sistema Iniciado\n");
            ...
            case WAIT:
                ...
                if (rg_counter_sampling == 0u) 
                {
                    rg_counter_sampling = SAMPLING_PERIOD_MS;
                    
                    co_adc = ADC_GetValue(); // Leer valor del sensor de CO
            ...
            case ALARM:
                ...
                /* Samplear continuamente CO durante la alarma */
                if (rg_counter_sampling == 0u) {
                    rg_counter_sampling = SAMPLING_PERIOD_MS;
                ...
                /* Enviar SMS solo la primera vez que se activa la alarma */
                if (alarm_sms_sent == 0u && co_adc > CO_ADC_UMBRAL) {
                    enviar_sms_uart((uint8_t *)"AT CO alto\n");
                    alarm_sms_sent = 1u;
                    rg_counter_sms = 120000u;  // Cargar 2 minutos
                }
                /* Reenviar cada 2 minutos mientras persista la alarma */
                else if (rg_counter_sms == 0u && co_adc > CO_ADC_UMBRAL) {
                    enviar_sms_uart((uint8_t *)"AT CO alto\n");
                    rg_counter_sms = 120000u;  // Recargar 2 minutos
                }

                else if (rg_counter_sms == 0u && co_adc < CO_ADC_UMBRAL){
                    enviar_sms_uart((uint8_t *)"AT CO normal\n");
                    rg_counter_sms = 120000u;  // Recargar 2 minutos para evitar reenvío inmediato si el CO vuelve a subir
                }

                /* Muestreo del botón para desactivar alarma */
                if (switch_debounced(GPIOB, PIN_BUTTON) == pressed && co_adc < CO_ADC_UMBRAL) {
                    GPIO_WritePin(GPIOB, PIN_LED_ALARMA, LED_OFF);
                    GPIO_WritePin(GPIOB, PIN_RELAY, RELAY_OFF);
                    state = WAIT;
                    alarm_sms_sent = 0u;
            ...
            case ERROR:
                ...
                enviar_sms_uart((uint8_t *)"AT Error\n");
                state = WAIT;
                break;
